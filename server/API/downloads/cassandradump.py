...
"""   File Name   :   cassandradump.py                                                                               """

"""   Description :   cassandradump.py                                                                               """

"""                                                                                                                  """

"""   Date :   09/06/2016                                          Created By  :   Thoughtgrains Solutions           """
...
def table_to_cqlfile(session, keyspace, tablename, flt, tableval, filep):
    ...
    def make_row_encoder(tableevel):
        partitions = dict(
            ...
            for has_counter, columns in itertools.groupby(tableval.columns.iteritems())
        )
...
def setup_cluster():
    ...
    if args.protocol_version is not None:
        ...
        cluster = Cluster(contact_points=nodes, protocol_version=args.protocol_version, auth_provider=auth,
                          load_balancing_policy=cassandra.policies.WhiteListRoundRobinPolicy(nodes))
    else:
        cluster = Cluster(contact_points=nodes, port=port,
                          load_balancing_policy=cassandra.policies.WhiteListRoundRobinPolicy(nodes))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='A data exporting tool for Cassandra inspired from mysqldump, with some added slice and dice capabilities.')
    ...
    parser.add_argument('--filter',
                        help='export a slice of a column family according to a CQL filter. This takes essentially a typical SELECT query stripped of the initial "SELECT ... FROM" part (e.g. "system.schema_columns where keyspace_name =\'OpsCenter\'", and exports only that data. Can be specified multiple times',
                        action='append')
    ...
    parser.add_argument('--keyspace',
                        help='export a keyspace along with all its column families. Can be specified multiple times',
                        action='append')
